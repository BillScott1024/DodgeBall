...
public class KinectModelControllerV2 : MonoBehaviour
{
    ...
    private float Foot_Left_pos;
    private float Foot_Right_pos;
    private float Spine_pos;
    ...
    void Start()
    {
        ...
        Spine_pos = Spine.transform.position.x;
    ...
    void Update()
    {
        ...
        if (sw.pollSkeleton())
        {
            for (int ii = 0; ii < (int)Kinect.NuiSkeletonPositionIndex.Count; ii++)
            {
                ...
                {
                    RotateJoint(ii);
                    if (ii == (int)Kinect.NuiSkeletonPositionIndex.FootLeft)
                    {

                        if (Mathf.Abs(Foot_Left.transform.position.x - Foot_Left_pos)> 0.05f || (Mathf.Abs(Foot_Right.transform.position.x - Foot_Right_pos)) > 0.05f)
                        {
                            if (Foot_Left.transform.position.x - Foot_Left_pos < 0)
                            {
                                Robot.transform.position += new Vector3(-0.1f, 0f, 0f);
                            }
                            if (Foot_Right.transform.position.x - Foot_Right_pos > 0)
                            {
                                Robot.transform.position -= new Vector3(-0.1f, 0f, 0f);
                            }
                            //else
                            ...
                            //}
                            // Robot.GetComponent<Animation>().Play("run");
                        ...
                        {
                            //if (Mathf.Abs(Foot_Right.transform.position.x - Foot_Right_pos) > 0.15f)
                            //{
                            //    if (Foot_Right.transform.position.x - Foot_Right_pos > 0)
                            //    {
                            //        Robot.transform.position -= new Vector3(-0.1f, 0f, 0f);
            ...
            //			if(Foot_Left.transform.position.z < Foot_Left_pos)
            //			{
